...
using BulkMessagesWebServer.Tests.Integrations;
using BulkMessagesWebServer.DBObjects.MessengerDialog;

namespace BulkMessagesWebServer.Tests.Integrations
{
    public class SQLMassMessagesRepositoryTests : IntegrationTest
    {
        public SQLMassMessagesRepositoryTests() : base("TestDB") { }
        //--------------------------------------------------------------------------------------
        [Fact]
        public void Can_Add_New_Record_In_MES_UMNICO_SENDER_Entity()
        {
            IMassMessagesRepository sut = new SqlMassMessagesRepository(m_ConnectionString);
